...
const RefNameField = ({ register, watch, i }: FieldProps) => {
  ...
  return register ? (
    <TextField
      ...
      placeholder={t("what-is-his-name")}
      InputLabelProps={{
        className: "subItem",
        sx: {
          "&.Mui-focused": {
          ...
          "&:not(.Mui-focused)": {
            transform:
              watch && (xs || sm) && watch(`contacts[${i}].name`)
                ? "translate(9px,2px) scale(0.75)"
                : watch && watch(`contacts[${i}].name`)
                  ? "translate(14px,-11px) scale(0.75)"
                  : watch && (xs || sm) && !watch(`contacts[${i}].name`)
